...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    help_text = (
        ...
        "<b>/set_server</b> - Configurar servidor (ip, port e user) (todos os parâmetros são obrigatórios)\n"
        "<b>/server_info</b> - Mostrar informações do servidor configurado para este chat\n"
        "<b>/grant_user &amp; /revoke_user &lt;id&gt;</b> - (ADMIN)\n"
        "<b>/grant_group &amp; /revoke_group &lt;id&gt;</b> - (ADMIN)\n\n"
        "Observações:\n\n"
        "- Em chats privados, todas as mensagens são direcionadas ao bot.\n"
        "- Em grupos, é necessário mencionar o bot (por @ ou com o padrão 'aivis bot') para que ele inicie a interação.\n"
        "- Antes de utilizar o bot (exceto /help), configure o servidor usando <b>/set_server</b>.\n\n"
        "Para gerar uma chave pública em um servidor Linux, execute:\n"
        "  ssh-keygen -t rsa -b 4096 -C \"seu_email@dominio.com\"\n"
        "Depois, use:\n"
        "  cat ~/.ssh/id_rsa.pub\n\n"
        "Esta é a chave pública que o bot usa para autenticação SSH (bot_key.pub):\n\n"
        f"<pre>{key_content}</pre>\n\n"
        "Adicione o conteúdo acima ao arquivo <i>~/.ssh/authorized_keys</i> do servidor de destino.\n\n"
